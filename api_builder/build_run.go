...
func (ctx *Context) BuildRun() {
	...
	builder.CloseBracket()
	builder.AddIf("ctx.DownloadRefreshRate == 0")
	builder.SetVarValue("ctx.DownloadRefreshRate", "time.Millisecond * 200").AddLine()
	builder.CloseBracket()
	builder.SetVarValue("ctx.isRunning", "true").AddLine()
	...
	builder.CloseBracket()
	builder.InitVarValue("me", "res.Result.(types.User)").AddLine()
	builder.SetVarValue("ctx.me", "&me").AddLine()
	...
	builder.CloseBracket()
	builder.AddIf("err != nil")
	builder.AddLog("Printf", "\"[%d] Retrying \\\"getUpdates\\\" due to Telegram says %s\", ctx.me.ID, err").AddLine()
